...
export function getPaths(env = process.env, platform = process.platform) {
  ...
  return {
    home,
    repositoryDir: posix.join(dataHome, 'session-memory'),
    configFile: posix.join(configHome, 'session-memory', 'config.json'),
    claudeDir: posix.join(home, '.claude'),
    codexSkillsDir: posix.join(home, '.agents', 'skills'),
  };
}
