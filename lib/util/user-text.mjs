...
const COMMAND_TAG = /<command-(name|message|args)>[\s\S]*?<\/command-\1>/gi;

// A slash/`$` command row is unwrapped to its command text only when the command tags are the
// ENTIRE message. A `<command-name>` tag embedded in ordinary prose is user content, not a control
// turn: honoring it would let a crafted transcript row silently exclude every turn that follows it
// from the canonical events and the tamper-evident content hash.
function unwrapCommandText(text) {
  const original = String(text ?? '');
  if (!/<command-name>/i.test(original)) return original;
  if (original.replace(COMMAND_TAG, '').trim().length > 0) return original;
