...
// Best-effort read for the read-only legacy digest matchers: a corrupt digest is unusable for
// matching anyway, so skip it rather than let one bad file abort save/read across the checkout.
...
function loadV4Revision(projectRoot, file) {
...
function loadV3Revision(projectRoot, project, file) {
...
// The logical id is derivable from a revision file's path, so bucket by it before parsing:
// an unreadable file then quarantines only its own logical session instead of aborting every
// save/read/list/get in the checkout. Availability for other sessions is restored while the
// affected session still fails closed (its integrity is never silently patched over).
function bucketByLogical(files) {
  const buckets = new Map();
  for (const file of files) {
    const logicalId = path.basename(path.dirname(path.dirname(file)));
    if (!buckets.has(logicalId)) buckets.set(logicalId, []);
    buckets.get(logicalId).push(file);
  }
  return buckets;
}

function loadV4(projectRoot) {
  const root = path.join(projectRoot, 'session-history', V4_DIR, 'sessions');
  const files = walk(root, (name, full) => name.endsWith('.json') && full.includes(`${path.sep}revisions${path.sep}`));
  const groups = new Map();
  const errors = new Map();
  for (const [logicalId, bucket] of bucketByLogical(files)) {
    try {
      groups.set(logicalId, bucket.map((file) => ({ ...loadV4Revision(projectRoot, file), source_schema: 4 })));
    } catch (error) {
      errors.set(logicalId, error instanceof Error ? error.message : String(error));
    }
  }
  return { groups, errors };
}

function loadV3(projectRoot) {
  ...
  const groups = new Map();
  const errors = new Map();
  if (files.length === 0) return { groups, errors };
  const buckets = bucketByLogical(files);
  let project;
  try {
    project = readJson(path.join(root, 'project.json'), 'schema-3 project');
    if (project.schema !== 3 || !project.project_id) throw new Error(`Invalid schema-3 project record in ${root}.`);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    for (const logicalId of buckets.keys()) errors.set(logicalId, message);
    return { groups, errors };
  }
  for (const [logicalId, bucket] of buckets) {
    try {
      groups.set(logicalId, bucket.map((file) => ({ ...loadV3Revision(projectRoot, project, file), source_schema: 3 })));
    } catch (error) {
      errors.set(logicalId, error instanceof Error ? error.message : String(error));
    }
  }
...
function storedSessionsDetailed(projectRoot, includeV3 = true) {
  assertSessionHistoryContained(projectRoot);
  const v4 = loadV4(projectRoot);
  const v3 = includeV3 ? loadV3(projectRoot) : { groups: new Map(), errors: new Map() };
  const groups = new Map(v3.groups);
  for (const [logicalId, revisions] of v4.groups) {
    groups.set(logicalId, [...(groups.get(logicalId) || []), ...revisions]);
  }
  const errors = new Map([...v3.errors, ...v4.errors]);
  const sessions = [];
  for (const [logicalId, revisions] of groups) {
    // A logical id broken on either schema side would merge into a partial DAG; skip it rather
    // than emit a plausible-but-incomplete session, and keep the error for the caller to surface.
    if (errors.has(logicalId)) continue;
    try {
  ...
  return { sessions, errors: [...errors].map(([logical_id, message]) => ({ logical_id, message })) };
}

function storedSessions(projectRoot, includeV3 = true) {
  return storedSessionsDetailed(projectRoot, includeV3).sessions;
}
...
export function loadLogicalSessionsDetailed(projectRoot) {
  return storedSessionsDetailed(projectRoot, true);
}
...
export function legacyImportMarkerCandidates(projectRoot, events) {
  ...
  for (const file of walk(digestRoot, (name) => name.endsWith('.json'))) {
    const digest = readJsonOrNull(file);
    if (!digest) continue;
...
export function saveSessionRevision({
  ...
  try {
    ...
    const stored = storedSessionsDetailed(projectRoot, true);
    const brokenTarget = stored.errors.find((entry) => entry.logical_id === logicalId);
    if (brokenTarget) {
      throw new Error(`Refusing to save into logical session ${logicalId}: its stored history is unreadable (${brokenTarget.message}). Repair or remove the offending file before saving.`);
    }
    const allSessions = stored.sessions;
