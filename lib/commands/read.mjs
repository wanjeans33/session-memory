...
function loadCandidates(root) {
  ...
  for (const session of loadLogicalSessions(root)) byLogicalId.set(session.logical_id, candidateFromSession(session));
  ...
  return [...byLogicalId.values()].sort((a, b) => String(b.digest.ended_at || b.digest.saved_at || '').localeCompare(String(a.digest.ended_at || a.digest.saved_at || '')));
}
...
export function read({
  ...
  let candidates = loadCandidates(root);
