...
// (CLI --resume + Desktop sidebar), prefixing titles with a source label like "(codex@alice) …".
export function read({ list = false, import: doImport = false, ids, targets = 'cli', author, cwd, projectsDir, desktopSessionsDir, env = process.env, platform = process.platform } = {}) {
  ...
  // --list (walks digests/<author>/ subdirectories and legacy flat files alike)
  if (!doImport) {
    const rows = [];
    for (const file of walk(digestDir, (n) => n.endsWith('.json'))) {
      let d;
      try {
  ...
  for (const baseName of wantBases) {
    const dp = walk(digestDir, (n) => n === `${baseName}.json`)[0];
    if (!dp) {
      console.log(`Skipped (not found): ${baseName}`);
      continue;
    }
    const d = JSON.parse(readFileSync(dp, 'utf8'));
    const lbl = d.author ? `${label(d.tool)}@${d.author}` : label(d.tool);
    ...
    if (srcTs && (d.tool === 'claude-cli' || d.tool === 'claude-desktop')) {
      ...
      const note = `[Imported from ${d.tool} (${d.author ? `${d.author}@` : ''}${d.machine})] turns=${d.turns}. Redacted original transcript: ${d.transcript_ref}. Files changed: ${(d.files_touched || []).join(', ')}`;
      const a = {
        type: 'assistant',
