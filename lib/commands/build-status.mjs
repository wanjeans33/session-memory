  ...
  if (existsSync(digDir)) {
    for (const file of walkJson(digDir)) {
      ...
      try {
        ...
        // One unreadable legacy digest must not brick the whole status build; skip and warn.
        console.warn(`session-memory: skipped unreadable legacy digest ${file}: ${error instanceof Error ? error.message : String(error)}`);
        continue;
      }
