...
function identityEnv(env, options) {
  return {
    ...env,
    ...(options.author ? { SESSION_MEMORY_AUTHOR: options.author } : {}),
    ...(options.actor ? { SESSION_MEMORY_ACTOR_ID: options.actor } : {}),
    ...(options.device ? { SESSION_MEMORY_DEVICE_ID: options.device } : {}),
    ...(options.role ? { SESSION_MEMORY_ROLE: options.role } : {}),
  };
}
  ...
  switch (command) {
    ...
    case 'save':
      return save({
        ...
        cwd: options.cwd,
        env: {
          ...identityEnv(env, options),
          ...(options['codex-session-id'] ? { CODEX_THREAD_ID: options['codex-session-id'] } : {}),
        },
      });
    case 'read':
      return read({
        ...
        codexSessionsDir: options['sessions-dir'],
        env: identityEnv(env, options),
      });
    case 'repo-status':
      return repoStatus({ repo: options.repo });
    case 'build-status': {
      const rawDays = options.days;
      if (rawDays !== undefined && !/^\d+$/.test(String(rawDays))) {
        throw new Error('--days must be a non-negative whole number.');
      }
      const days = rawDays === undefined ? 0 : Number(rawDays);
      if (!Number.isSafeInteger(days)) {
        throw new Error('--days must be a non-negative safe integer.');
      }
      return buildStatus({ repo: options.repo, days });
    }
    default:
